...
const FloristDocumnet = (props) => {
        ...
        apiService.get('/order/admin/sales/'+start_date+'/'+end_date+'?login='+user.user.login).then(
          (response) =>  {
            ...
            apiService.get('/ordercomposition/admin/sales/'+start_date+'/'+end_date+'?login='+user.user.login).then( 
              ...
              data = response;
              console.log(data);
              apiService.post('/document/florist/maded', {start_date:start_date,end_date:end_date,date: data,all:all})
              .then(
                props.getDocument('/document/maded')
              )
             })
            })
      }
  ...
  const handleDateChangeEnd = (date) => {
    ...
    return(
      <div>
        <h3 className='d-flex justify-content-center align-items-center' style={{color:'#FE3B6D'}}>Отчёт об изготовленных букетах</h3>
...
export default FloristDocumnet;